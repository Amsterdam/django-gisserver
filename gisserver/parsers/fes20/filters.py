...
class Filter(BaseNode):
    ...
    def from_kvp_request(cls, kvp: KVPRequest) -> Filter | None:
        ...
        bbox = kvp.get_custom("bbox", default=None, parser=parse_bbox)
...
def parse_bbox(bbox_value: str):
    ...
    polygon = Polygon.from_bbox((float(bbox[0]), float(bbox[1]), float(bbox[2]), float(bbox[3])))
    if len(bbox) == 5:
    ...
    return _BBoxPolygon(srs=crs, geos_data=polygon)


class _BBoxPolygon(GEOSGMLGeometry):
    """The value from a BBOX parameter.
    Basically a gml:Polygon, but extended here to support delayed CRS resolving.
    """
    ...
    def build_rhs(self, compiler):
        ...
        return super().build_rhs(compiler)
