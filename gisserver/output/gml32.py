...
class GML32Renderer(OutputRenderer):
    ...
    def write_gml_field(
        ...
        if xsd_element.type is XsdTypes.gmlBoundingShapeType:
            ...
            value = cast(GmlBoundedByElement, xsd_element).get_value(instance, crs=self.output_crs)
...
class DBGML32Renderer(DBGMLRenderingMixin, GML32Renderer):
    ...
    def decorate_queryset(
        ...
        **params,
        ...
        if geo_selects:
            as_gml_map = build_db_annotations(geo_selects, "_as_gml_{name}", AsGML)
            queryset = queryset.defer(*geo_selects.keys()).annotate(**as_gml_map)
    ...
    def get_prefetch_queryset(
        ...
        geo_selects = get_db_geometry_selects(
            feature_relation.geometry_elements, output_crs, relative_path=True
        )
        if geo_selects:
            # Exclude geometries from the fields, fetch them as pre-rendered annotations instead.
            as_gml_map = build_db_annotations(geo_selects, "_as_gml_{name}", AsGML)
            return base.defer(*geo_selects.keys()).annotate(**as_gml_map)
        else:
            return base
    ...
    def write_gml_field(
        ...
        if xsd_element.type is XsdTypes.gmlBoundingShapeType:
            ...
            value = get_db_annotation(instance, xsd_element, "_as_gml_{name}")
            if value is None:
...
class GML32ValueRenderer(GML32Renderer):
    ...
    def write_gml_field(
        ...
        gml_id = self.get_gml_id(xsd_element.source.model._meta.object_name, instance["pk"])
        gml = self.render_gml_value(gml_id, value, extra_xmlns=extra_xmlns)
        self._write(f"<{xml_name}{extra_xmlns}>{gml}</{xml_name}>\n")
