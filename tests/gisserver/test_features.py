...
class TestFeatureField:
    ...
    def test_as_xml(self, field_name, cls_type, xsd_type, xml):
        ff = FeatureField(field_name, model_attribute=field_name, model=Restaurant)
...
class TestGeneratedFields:
    ...
    def test_generated_field_is_resolved_correctly(self, field_name, type, xml):
        ff = FeatureField(field_name, model_attribute=field_name, model=ModelWithGeneratedFields)
        assert ff.xsd_element.as_xml == xml
    ...
    def test_generated_geometry_field(self):
        ...
        assert model_fields == [
            ModelWithGeneratedFields.geometry.field,
            ModelWithGeneratedFields.geometry_translated.field,
        ]
