...
@pytest.mark.django_db
class TestGetCapabilitiesWithPostRequest:
    """All tests for the GetCapabilities method."""

    def test_post(self, client, restaurant, coordinates):
        """Prove that the happy flow works"""
        xml = f'<GetCapabilities service="WFS" acceptversions="2.0.0" {XML_NS}></GetCapabilities>'
    ...
    def test_version_negotiation(self, client):
        """Prove that version negotiation still returns 2.0.0"""
        xml = '<GetCapabilities service="WFS" acceptVersions="1.0.0,2.0.0"></GetCapabilities>'
        response = client.post("/v1/wfs/", data=xml, content_type="application/xml")
        content = response.content.decode()
        assert response["content-type"] == "text/xml; charset=utf-8", content
        assert response.status_code == 200, content
    ...
    def test_get_invalid_version(self, client):
        """Prove that version negotiation works"""
        xml = '<GetCapabilities service="WFS" acceptVersions="1.5.0"></GetCapabilities>'
        response = client.post("/v1/wfs/", data=xml, content_type="application/xml")
        content = response.content.decode()
        assert response["content-type"] == "text/xml; charset=utf-8", content
        assert response.status_code == 400, content
    ...
    def test_get_flattened(self, client, restaurant, coordinates):
        xml = '<GetCapabilities service="WFS" acceptVersions="2.0.0"></GetCapabilities>'
