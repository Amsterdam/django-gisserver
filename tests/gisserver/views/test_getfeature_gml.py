...
class TestGetFeatureWithPostRequest:
    """
    Tests for GetFeature with a POST request.
    """
   ...
   timeStamp="{timestamp}" numberMatched="1" numberReturned="1">
    ...
    def test_post_empty_geometry(self, client):
        """Prove that the empty geometry values don't crash the rendering."""
        restaurant = Restaurant.objects.create(name="Empty")
        xml = f"""<GetFeature service="WFS" version="2.0.0" {XML_NS}>
              <Query typeNames="restaurant">
    ...
    def test_post_limited_fields(self, client, restaurant, coordinates):
        """Prove that the 'FeatureType(fields=..)' reduces the returned fields."""
        xml = f"""<GetFeature service="WFS" version="2.0.0" {XML_NS}>
   ...
   timeStamp="{timestamp}" numberMatched="1" numberReturned="1">
    ...
    def test_post_complex(self, client, restaurant_m2m, bad_restaurant, coordinates):
        """Prove that rendering complex types works."""
        xml = f"""<GetFeature service="WFS" version="2.0.0" {XML_NS}>
              <Query typeNames="restaurant">
              </Query>
   ...
   timeStamp="{timestamp}" numberMatched="2" numberReturned="2">
    ...
    def test_post_flattened(self, client, restaurant, bad_restaurant, coordinates):
        """Prove that rendering complex types works."""
        xml = f"""<GetFeature service="WFS" version="2.0.0" {XML_NS}>
              <Query typeNames="restaurant">
              </Query>
...
""",  # noqa: E501
    ...
    def test_post_srs_name(self, client, restaurant, coordinates):
        """Prove that specifying SRSNAME works"""
        xml = f"""<GetFeature service="WFS" version="2.0.0" srsName="urn:ogc:def:crs:EPSG::28992" {XML_NS}>
              <Query typeNames="restaurant">
              </Query>
              </GetFeature>
              """
    ...
    @pytest.mark.parametrize("use_count", [1, 0])
    def test_post_hits(self, client, restaurant, use_count, monkeypatch):
        """Prove that that parsing RESULTTYPE=hits works"""
        monkeypatch.setattr(conf, "GISSERVER_COUNT_NUMBER_MATCHED", use_count)
        xml = f"""<GetFeature service="WFS" version="2.0.0" resultType="hits" {XML_NS}>
              <Query typeNames="restaurant">
              </Query>
              </GetFeature>
              """
   ...
   timeStamp="{timestamp}" numberMatched="1" numberReturned="0">
    ...
    def test_post_truncated_response(self, client, restaurant, monkeypatch):
        """Prove that errors are properly handled during streaming."""
