...
* Added `WFSView.xml_namespaces` attribute to configure custom namespace prefixes.
...
* Request parsing:
  ...
  * `WFSOperation.parser_class` allows to define a custom parser for a additional WFS methods.
  * The WFS request parsing moves to `gisserver.parsers.wfs20`, which builds an Abstract Syntax Tree (AST) of the XML request.
